  ...
  return (
    <div className="mt-16">
      ...
      <ul className="flex flex-wrap flex-col lg:flex-row gap-4 justify-center text-white">
        {projects.map(({ title, subtitle, skills }, i) => {
          return (
            // TODO: replace with project id
            <Link key={`project-${title}`} href={`/project/{${title}}`}>
              <li
                className={`basis-5/12 lg py-32 bg-[#253956] flex flex-col items-center justify-center p-4 text-center rounded-tr-[4rem] select-none cursor-pointer rounded-bl-[4rem] ${
